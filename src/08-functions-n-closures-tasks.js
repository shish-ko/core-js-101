...
function retry(func, attempts) {
  ...
  return () => {
    const result = res(func);
    return (result instanceof Error && attempts > 1) ? retry(func, attempts - 1)() : result;
  };
}
...
function logger(func, logFunc) {
  function res(...args) {
    ...
    const resStr = str.slice(1, str.length - 1);
    logFunc(`${func.name}(${resStr}) starts`);
    try {
      return func(...args);
    } catch (e) {
      throw Error(e);
    } finally {
      logFunc(`${func.name}(${resStr}) ends`);
    }
  }
  return res;
}
